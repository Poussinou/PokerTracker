...
import com.github.saikcaskey.pokertracker.presentation.RootComponent.Child
import com.github.saikcaskey.pokertracker.presentation.RootComponent.Child.*
...
class RootComponentImpl(
    ...
    private fun buildChildComponent(config: Config, context: ComponentContext): Child =
        when (config) {
            is Config.Main -> Main(mainComponent(context))
            is Config.DayDetail -> PlannerDayDetail(dayDetailComponent(context, config.date))
            ...
            is Config.ViewEvents -> ViewEvents(viewEventsComponent(context))
            is Config.ViewVenues -> ViewVenues(viewVenuesComponent(context))
            is Config.ViewExpenses -> ViewExpenses(viewExpensesComponent(context))
            is Config.ExpenseDetail -> {
                ExpenseDetail(expenseDetailComponent(context, config.expenseId))
            }
        }
    ...
    private fun mainComponent(componentContext: ComponentContext): MainComponent =
        MainComponentImpl(
            ...
            onShowInsertEvent = { id, venueId, date ->
                navigator.push(Config.InsertEvent(id, venueId, date))
            },
            onShowInsertExpense = { id, venueId, eventId ->
                navigator.push(Config.InsertExpense(id, venueId, eventId))
            },
            onShowCalendarDayDetail = { date, hasEvent ->
                navigator.push(
                    if (hasEvent) Config.DayDetail(date) else Config.InsertEvent(null, null, date)
                )
            },
        )
